...
pub fn tag(ours: &StaticSecret, theirs: &PublicKey, ctr: u8, msg: &[u8]) -> [u8; 32] {
    ...
    compute_hmac(&secret, &ours.into(), theirs, ctr, msg)
        .finalize()
        .into_bytes()
        .into()
}

pub fn verify(ours: &StaticSecret, theirs: &PublicKey, ctr: u8, msg: &[u8], tag: &[u8]) -> bool {
    let secret = ours.diffie_hellman(theirs);

    compute_hmac(&secret, theirs, &ours.into(), ctr, msg)
